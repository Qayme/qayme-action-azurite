...
class MessagesHandler extends BaseHandler_1.default {
    ...
    async dequeue(options, context) {
        ...
        const popReceipt = utils_1.getPopReceipt(context.startTime);
        // Validate the query parameters.
        const timeNextVisible = new Date(context.startTime.getTime() + constants_1.DEFUALT_DEQUEUE_VISIBILITYTIMEOUT * 1000 // 30s as default, convert to ms
        );
        ...
        for (const message of messages) {
            ...
            const text = await utils_1.readStreamToString(textStream);
            const dequeuedMessage = Object.assign(Object.assign({}, message), { messageText: text });
    ...
    async enqueue(queueMessage, options, context) {
        ...
        if (queueMessage.messageText === undefined) {
            ...
            const parsedBody = await utils_1.parseXMLwithEmpty(body || "");
            for (const text in parsedBody) {
                if (parsedBody.hasOwnProperty(text) &&
        ...
        if (utils_1.getUTF8ByteSize(queueMessage.messageText) > constants_1.MESSAGETEXT_LENGTH_MAX) {
        ...
        const message = {
            ...
            expirationTime: new Date(context.startTime.getTime() + constants_1.DEFUALT_MESSAGETTL * 1000),
            dequeueCount: 0,
            timeNextVisible: new Date(context.startTime),
            popReceipt: utils_1.getPopReceipt(context.startTime),
