...
const openAsync = util_1.promisify(fs_1.open);
const closeAsync = util_1.promisify(fs_1.close);
/**
...
class FSExtentStore {
    ...
    async clean() {
        if (this.isClosed()) {
            for (const path of this.persistencyConfiguration) {
                try {
                    await utils_1.rimrafAsync(path.locationPath);
                }
                catch (_a) {
    ...
    async appendExtent(data, contextId) {
        const op = () => new Promise((resolve, reject) => {
            (async () => {
                ...
                const ws = fs_1.createWriteStream(path, {
                    fd,
                    autoClose: false
                });
    ...
    generateExtentPath(persistencyId, extentId) {
        ...
        return path_1.join(directoryPath, extentId);
    }
}
