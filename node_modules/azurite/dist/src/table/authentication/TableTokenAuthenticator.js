...
class TableTokenAuthenticator {
    ...
    async validate(req, context) {
        ...
        this.logger.info(`TableTokenAuthenticator:validate() Start validation against token authentication.`, tableContext.contextID);
        // TODO: Make following async
        const accountProperties = this.dataStore.getAccount(account);
        if (accountProperties === undefined) {
            this.logger.error(`TableTokenAuthenticator:validate() Invalid storage account ${account}.`, tableContext.contextID);
        ...
        if (authHeaderValue === undefined) {
            this.logger.info(`TableTokenAuthenticator:validate() Request doesn't include valid authentication header. Skip token authentication.`, tableContext.contextID);
            return;
        }
        if (tableContext.operation === operation_1.default.Table_GetAccessPolicy ||
            tableContext.operation === operation_1.default.Table_SetAccessPolicy) {
            this.logger.info(`TableTokenAuthenticator:validate() Operation is not available with OAuth. Skip token authentication.`, tableContext.contextID);
            return;
        }
        if (!authHeaderValue.startsWith(constants_1.BEARER_TOKEN_PREFIX)) {
            throw StorageErrorFactory_1.default.getInvalidAuthenticationInfo(context);
        }
        if (req.getProtocol().toLowerCase() !== constants_1.HTTPS) {
            throw StorageErrorFactory_1.default.getAuthenticationFailed(context, "Authentication scheme Bearer is not allowed with HTTP.");
        }
        ...
        switch (this.oauth) {
            ...
            default:
                this.logger.warn(`TableTokenAuthenticator:validate() Unknown OAuth level ${this.oauth}. Skip token authentication.`, tableContext.contextID);
                return;
        }
    }
    async authenticateBasic(token, context) {
        ...
        try {
            ...
            throw StorageErrorFactory_1.default.getAuthenticationFailed(context, "Authentication scheme Bearer is not supported.");
        }
        if (!decoded) {
            throw StorageErrorFactory_1.default.getAuthenticationFailed(context, "Authentication scheme Bearer is not supported.");
        }
        ...
        if (now < nbf) {
            throw StorageErrorFactory_1.default.getAuthenticationFailed(context, "Lifetime validation failed.");
        }
        if (now > exp) {
            throw StorageErrorFactory_1.default.getAuthenticationFailed(context, "Lifetime validation failed. The token is expired.");
        }
        ...
        if (!issMatch) {
            throw StorageErrorFactory_1.default.getAuthenticationFailed(context, "Invalid token issuer.");
        }
        ...
        let audMatch = false;
        let m;
        for (const regex of constants_2.VALID_TABLE_AUDIENCES) {
            m = regex.exec(aud);
            if (m !== null) {
                if (m[0] === aud) {
        ...
        if (!audMatch) {
            throw StorageErrorFactory_1.default.getAuthenticationFailed(context, "Invalid token audience.");
        }
        // Skip validate scope
        // Currently scope maybe null or user_impersonation
        this.logger.info(`TableTokenAuthenticator:authenticateBasic() Validation against token authentication successfully.`, tableContext.contextID);
        return true;
    }
}
exports.default = TableTokenAuthenticator;
//# sourceMappingURL=TableTokenAuthenticator.js.map