...
async function main() {
    ...
    const config = new TableConfiguration_1.default(env.tableHost(), env.tablePort(), path_1.join(location, constants_1.DEFAULT_TABLE_LOKI_DB_PATH), (await env.debug()) !== undefined, !env.silent(), undefined, await env.debug(), env.loose(), env.skipApiVersionCheck(), env.cert(), env.key(), env.pwd(), env.oauth(), env.disableProductStyleUrl());
