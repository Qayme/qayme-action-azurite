...
class BlobServerFactory {
    async createServer(blobEnvironment) {
        ...
        if (!isVSC) {
            ...
            constants_3.DEFAULT_BLOB_PERSISTENCE_ARRAY[0].locationPath = path_1.join(location, constants_2.DEFAULT_BLOB_PERSISTENCE_PATH);
            ...
            if (isSQL) {
                ...
                const config = new BlobConfiguration_1.default(env.blobHost(), env.blobPort(), path_1.join(location, constants_3.DEFAULT_BLOB_LOKI_DB_PATH), path_1.join(location, constants_3.DEFAULT_BLOB_EXTENT_LOKI_DB_PATH), constants_3.DEFAULT_BLOB_PERSISTENCE_ARRAY, !env.silent(), undefined, debugFilePath !== undefined, debugFilePath, env.loose(), env.skipApiVersionCheck(), env.cert(), env.key(), env.pwd(), env.oauth(), env.disableProductStyleUrl());
