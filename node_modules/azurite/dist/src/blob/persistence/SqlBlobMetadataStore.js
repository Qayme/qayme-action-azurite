...
class SqlBlobMetadataStore {
    ...
    async init() {
        ...
        BlobsModel.init({
            ...
            committedBlocksInOrder: {
                type: sequelize_1.TEXT({ length: "medium" })
            },
            metadata: {
                type: "VARCHAR(2047)"
            }
    ...
    async setServiceProperties(context, serviceProperties) {
        return this.sequelize.transaction(async (t) => {
            ...
            if (findResult === null) {
                await ServicesModel.create(Object.assign({ accountName: serviceProperties.accountName }, updateValues), { transaction: t });
            }
            else {
    ...
    async deleteContainer(context, account, container, options = {}) {
        await this.sequelize.transaction(async (t) => {
            ...
            WriteConditionalHeadersValidator_1.validateWriteConditions(context, options.modifiedAccessConditions, findResult ? this.convertDbModelToContainerModel(findResult) : undefined);
            ...
            await BlobsModel.update({
                deleting: sequelize_1.literal("deleting + 1")
            }, {
                where: {
                    accountName: account,
                    containerName: container
    ...
    async setContainerMetadata(context, account, container, lastModified, etag, metadata, leaseAccessConditions, modifiedAccessConditions) {
        return this.sequelize.transaction(async (t) => {
            ...
            WriteConditionalHeadersValidator_1.validateWriteConditions(context, modifiedAccessConditions, findResult ? this.convertDbModelToContainerModel(findResult) : undefined);
            if (findResult === null || findResult === undefined) {
                throw StorageErrorFactory_1.default.getContainerNotFound(context.contextId);
            }
    ...
    async setContainerACL(context, account, container, setAclModel) {
        await this.sequelize.transaction(async (t) => {
            ...
            WriteConditionalHeadersValidator_1.validateWriteConditions(context, setAclModel.modifiedAccessConditions, findResult ? this.convertDbModelToContainerModel(findResult) : undefined);
            if (findResult === null || findResult === undefined) {
                throw StorageErrorFactory_1.default.getContainerNotFound(context.contextId);
            }
    ...
    async createBlob(context, blob, leaseAccessConditions, modifiedAccessConditions) {
        return this.sequelize.transaction(async (t) => {
            ...
            WriteConditionalHeadersValidator_1.validateWriteConditions(context, modifiedAccessConditions, blobFindResult
    ...
    async downloadBlob(context, account, container, blob, snapshot = "", leaseAccessConditions, modifiedAccessConditions) {
        return this.sequelize.transaction(async (t) => {
            ...
            ReadConditionalHeadersValidator_1.validateReadConditions(context, modifiedAccessConditions, blobFindResult
                ? this.convertDbModelToBlobModel(blobFindResult)
                : undefined);
    ...
    async listBlobs(context, account, container, blob, prefix = "", maxResults = constants_2.DEFAULT_LIST_BLOBS_MAX_RESULTS, marker, includeSnapshots, includeUncommittedBlobs) {
        return this.sequelize.transaction(async (t) => {
            await this.assertContainerExists(context, account, container, t);
            const whereQuery = {
                accountName: account,
    ...
    async commitBlockList(context, blob, blockList, leaseAccessConditions, modifiedAccessConditions) {
        await this.sequelize.transaction(async (t) => {
            ...
            const commitBlockBlob = Object.assign(Object.assign({}, blob), { deleted: false, committedBlocksInOrder: selectedBlockList, properties: Object.assign(Object.assign({}, blob.properties), { creationTime, lastModified: blob.properties.lastModified || context.startTime, contentLength: selectedBlockList
                        .map((block) => block.size)
                        .reduce((total, val) => {
                        return total + val;
                    }, 0), blobType: Models.BlobType.BlockBlob }) });
    ...
    async createSnapshot(context, account, container, blob, leaseAccessConditions, metadata, modifiedAccessConditions) {
        return this.sequelize.transaction(async (t) => {
            ...
            const snapshotTime = utils_1.convertDateTimeStringMsTo7Digital(context.startTime.toISOString());
    ...
    async deleteBlob(context, account, container, blob, options) {
        await this.sequelize.transaction(async (t) => {
            ...
            WriteConditionalHeadersValidator_1.validateWriteConditions(context, options.modifiedAccessConditions, blobFindResult
                ? this.convertDbModelToBlobModel(blobFindResult) // TODO: Reduce double convert
                : undefined);
            if (blobFindResult === null || blobFindResult === undefined) {
                throw StorageErrorFactory_1.default.getBlobNotFound(context.contextId);
    ...
    async setBlobHTTPHeaders(context, account, container, blob, leaseAccessConditions, blobHTTPHeaders, modifiedAccessConditions) {
        return this.sequelize.transaction(async (t) => {
            ...
            blobModel.properties.etag = utils_2.newEtag();
    ...
    setBlobMetadata(context, account, container, blob, leaseAccessConditions, metadata, modifiedAccessConditions) {
        return this.sequelize.transaction(async (t) => {
            ...
            const etag = utils_2.newEtag();
            await BlobsModel.update(Object.assign({ metadata: this.serializeModelValue(metadata) || null, lastModified,
                etag }, this.convertLeaseToDbModel(new BlobLeaseAdapter_1.default(blobModel))), {
                where: {
                    accountName: account,
                    containerName: container,
                    blobName: blob,
    ...
    startCopyFromURL(context, source, destination, copySource, metadata, tier, options = {}) {
        return this.sequelize.transaction(async (t) => {
            ...
            options.sourceModifiedAccessConditions =
                options.sourceModifiedAccessConditions || {};
            ReadConditionalHeadersValidator_1.validateReadConditions(context, {
            ...
            WriteConditionalHeadersValidator_1.validateWriteConditions(context, options.modifiedAccessConditions, destBlob);
            if (destBlob) {
            ...
            const copiedBlob = {
                ...
                deleted: false,
                snapshot: "",
                properties: Object.assign(Object.assign({}, sourceBlob.properties), { creationTime: context.startTime, lastModified: context.startTime, etag: utils_2.newEtag(), leaseStatus: destBlob !== undefined
                        ? destBlob.properties.leaseStatus
                        : Models.LeaseStatusType.Unlocked, leaseState: destBlob !== undefined
                        ? destBlob.properties.leaseState
                        : Models.LeaseStateType.Available, leaseDuration: destBlob !== undefined
                        ? destBlob.properties.leaseDuration
                        : undefined, copyId: v4_1.default(), copyStatus: Models.CopyStatusType.Success, copySource, copyProgress: sourceBlob.properties.contentLength
                        ? `${sourceBlob.properties.contentLength}/${sourceBlob.properties.contentLength}`
                        : undefined, copyCompletionTime: context.startTime, copyStatusDescription: undefined, incrementalCopy: false, destinationSnapshot: undefined, deletedTime: undefined, remainingRetentionDays: undefined, archiveStatus: undefined, accessTierChangeTime: undefined }),
                metadata: metadata === undefined || Object.keys(metadata).length === 0
                    ? Object.assign({}, sourceBlob.metadata) : metadata,
