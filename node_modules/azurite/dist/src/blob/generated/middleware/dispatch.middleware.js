...
const UnsupportedRequestError_1 = tslib_1.__importDefault(require("../errors/UnsupportedRequestError"));
 ...
 * @returns {void}
 */
function dispatchMiddleware(context, req, next, logger) {
    ...
    let conditionsMet = -1;
    for (const key in operation_1.default) {
        if (operation_1.default.hasOwnProperty(key)) {
            const operation = parseInt(key, 10);
            const res = isRequestAgainstOperation(req, specifications_1.default[operation], context.dispatchPattern);
    ...
    if (context.operation === undefined) {
        const handlerError = new UnsupportedRequestError_1.default();
        logger.error(`DispatchMiddleware: ${handlerError.message}`, context.contextId);
        return next(handlerError);
    }
    logger.info(`DispatchMiddleware: Operation=${operation_1.default[context.operation]}`, context.contextId);
    next();
}
 ...
 */
function isRequestAgainstOperation(req, spec, dispatchPathPattern) {
    ...
    const path = spec.path
        ? spec.path.startsWith("/")
            ...
            : `/${spec.path}`
        : "/";
    if (!utils_1.isURITemplateMatch(
    // Use dispatch path with priority
    dispatchPathPattern !== undefined ? dispatchPathPattern : req.getPath(), path)) {
        return [false, metConditionsNum++];
    }
    ...
    for (const queryParameter of spec.queryParameters || []) {
        if (queryParameter.mapper.required) {
            ...
            if (queryParameter.mapper.type.name === "Enum" &&
    ...
    for (const headerParameter of spec.headerParameters || []) {
        if (headerParameter.mapper.required) {
            ...
            if (headerParameter.mapper.type.name === "Enum" &&
...
//# sourceMappingURL=dispatch.middleware.js.map